...
import com.lwc.repair3.rsa.AES;
import com.lwc.repair3.rsa.ConvertUtils;
import com.lwc.repair3.rsa.EncryUtil;
import com.lwc.repair3.rsa.RSA;
...
/*    @JmsListener(destination = "mytest.queue")
    ...
    @JmsListener(destination = "mytest.queue")
    public void receiveQueue(RSAData msg) throws Exception {
        // 验签通过
        String aeskey = RSA.decrypt(msg.getEncryptkey(),
                serverPrivateKey);
        String data = ConvertUtils.hexStringToString(AES.decryptFromBase64(msg.getData(),
                aeskey));
        System.out.println("解密后的明文1:data:" + data);
        SMS sms = JSON.parseObject(data,SMS.class);
        System.out.println("解密后的明文2:data:" + JSON.toJSONString(sms));
        SMS sms2 = JSON.parseObject(data, new TypeReference<SMS>() {});
        System.out.println("解密后的明文3:data:" + JSON.toJSONString(sms2));
        JSONObject jsonObj = JSONObject.parseObject(data);
        System.out.println("解密后的明文4:data:" +jsonObj.toString());
    }
}
