...
public class EncryUtil {
	...
	public static String handleRSA(TreeMap<String, Object> map,
		...
		for (Entry<String, Object> entry : map.entrySet()) {
			sbuffer.append(JSON.toJSONString(entry.getValue()));
		}
		String signTemp = sbuffer.toString();
		System.out.println("EncryUtil signTemp：");
	...
	public static boolean checkDecryptAndSign(String data, String encrypt_key,
		...
		try {
			...
			System.out.println("serverPrivateKey解开aesEncrypt AESKey：");
		...
		System.out.println("用aeskey解开data。取得data明文：");
		...
		System.out.println("取得data明文sign。：");
		...
		boolean result = RSA.checkSign(signData.toString(), sign,
				clientPublicKey);
		System.out.println("result为true时表明验签通过：");
